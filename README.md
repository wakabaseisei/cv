...
- 社内ソフトウェア開発研修の作問プロジェクトに参画。テスト・リファクタリングをテーマにして、実際の基幹システムのコードを使った研修内容を策定。当日のライブコーディングを実施。
　　- 既存の基幹システムにはほとんどテストコードが存在せず、対応するドキュメントも存在しないか最新化されていない状態でした。研修では実際の基幹システムのコードを扱って、
