...
- 各ECプラットフォームのAPIとの連携において、プラットフォーム固有の情報がドメインにまで侵入している状態でした(レスポンスの情報をそのままエンティティの構造体にデコードしているなど)。DTOを導入したことで、プラットフォーム固有の情報をその中に閉じ込めるようにして、ドメインの知識にそれらの詳細が及ばないようにしました。
