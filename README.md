...
④仕事の進め方<br/>
チーム全体としては、スクラム開発をベースに進めています。具体的には2週間のイテレーションのうちに、スプリントプランニングやバックログリファインメントなどの各種イベントを実施し、ユーザーストーリーごとにサブタスクを洗い出して、サブタスクごとにfeatureブランチを切って、実装を進めています。実装したものはレビュー依頼を出して、2人以上に承認を得たら、developブランチにマージする方針で進めています。
