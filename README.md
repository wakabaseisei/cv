...
④仕事の進め方<br/>
チーム全体としては、スクラム開発をベースにして進めています。2週間のイテレーションのうちに、スプリントプランニングやバックログリファインメントなどの各種イベントを実施しながら、開発を進めています。ユーザーストーリーごとにサブタスクを洗い出して、サブタスクごとにfeatureブランチを切って、実装を進めています。実装したものはレビュー依頼を出して、2人以上に承認を得たら、developブランチにマージする方針で進めています。
...
### 主な業務内容
- 登録可能数の上限を環境変数で設定できるように実現。決済マイクロサービスのAPI内の実装を変更。Helm(Kubernetes)のテンプレートファイルを変更し、環境ごとに上限値を変えられるように実現。
