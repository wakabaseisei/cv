...
それらを解決していくためのモダナイゼーションの機運が高まっている中で、受注ドメイン領域のマイクロサービス化、技術・アーキテクチャ刷新を担当しました。具体的には以下。
- 言語選定でGoを採用。既存の基幹システムはPythonで構築されていたが、前職でのGoの開発経験を活かし、Goの利点である仕様の小ささやマイクロサービスとの相性の良さなどを紹介しながら、Goの採用を推進。
- アプリケーションのアーキテクチャ設計。
- 初回開発スコープの選定
- シーケンス図の作成
...
　　- 特定の形式のタグのPushをトリガーとして、コンテナのイメージビルドと本番デプロイ依頼を出すワークフローを作成
　　- 本番デプロイ依頼を受けて、本番環境のリリースチケット起票を自動化するワークフローを作成
　　

また、基幹システムのモダナイズ活動とは別に、以下のような活動もしました。
- 社内ソフトウェア研修の問題作成プロジェクトに参画。テスト・リファクタリングをテーマにして、実際の基幹システムのコードを使用して、お題選定やライブコーディングなどを実施。
- その他、既存の基幹システムの新規機能追加、保守改修運用
