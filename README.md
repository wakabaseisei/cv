...
④仕事の進め方<br/>
・開発手法<br/>
チーム全体としては、スクラム開発をベースに進めています。具体的には2週間のイテレーションのうちに、スプリントプランニングやバックログリファインメントなどの各種イベントを実施します。<br/>
・開発の流れ<br/>
...
- 某決済種による取引の検索機能の実装
- 統合テストシナリオ作成
- GKEのアップデート時の可用性向上のための改善。Podの削除によるデータの不整合等が発生しない仕組みを構築。Pod上に起動しているアプリケーションのプロセスに通知されるSIGTERMからSIGKILLまでの間に、正常に終了するようなシグナルハンドリングを実装。
